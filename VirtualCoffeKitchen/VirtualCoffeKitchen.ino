...
void loop() {
  ...
  else if(totalPressTime>0 && totalPressTime<4000 && buttonPressed==1)
  {
    buttonPressed=0;
    totalPressTime=0;
  ...
  else
  {
    Heltec.display->clear();
    Heltec.display->setFont(ArialMT_Plain_10);
    Heltec.display->drawStringMaxWidth(0, 0, 128,"please switch to Access point mode to reconfigure the Wifi");    Heltec.display->display();
